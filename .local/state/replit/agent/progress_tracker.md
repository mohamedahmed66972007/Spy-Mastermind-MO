...
[x] 28. Fixed pre-voting transition timer frozen at 58 - Dec 9, 2024 - Reset timerRemaining to 10 when phase changes to pre_voting_transition in game-context.tsx. The old timer value from questioning phase was carrying over instead of being reset.
[x] 29. Fixed pre-voting transition timer not starting - Dec 9, 2024 - Added pre_voting_transition phase handling to done_with_questions and end_turn handlers in routes.ts. These handlers were only checking for spy_voting phase transition but not pre_voting_transition, so startPreVotingTransition() was never being called when the phase changed.
[x] 30. Fixed progress bar for spy voting and spy guessing - Dec 9, 2024 - Progress bars were using hardcoded 30 seconds instead of actual settings. Now uses room.gameSettings.spyVotingDuration and room.gameSettings.spyGuessDuration for accurate progress bar movement.
